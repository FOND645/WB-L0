...
class deliverySelector {
    constructor(args) {
        this.publicAddresses = args.publicAdresses
        this.privateAddresses = args.privateAddresses

        this.isCoisedAdressPublic = true
        ...
        this.initialButtonsElements.forEach(Element => Element.addEventListener('click', () => { }))

        // Модальное окно
        this.modalContainerElement = document.getElementById(args.modalContainerID)
        this.modalContainerElement.addEventListener('click', () => { })
        ...
        this.privatePointsCheckBoxesElements.forEach((Element, ind) => Element.addEventListener('click', () => { }))
        ...
        this.publicPointsCheckBoxesElements.forEach((Element, ind) => Element.addEventListener(() => { }))

        // Кнопка "Курьером"
        this.privatePointsSelectorElement = document.getElementById(args.privatePointsSelectorID)
        this.privatePointsSelectorElement.addEventListener('click', () => { })

        // Кнопка "В пункт выдачи"
        this.publicPointsSelectorElement = document.getElementById(args.publicPointsSelectorID)
        this.publicPointsSelectorElement.addEventListener('click', () => { })
    ...
    swichAddressType(isPublic) {
        return () => {
            ...
            this.privateAddressesElements.forEach(Element => Element.style.display = isPublic ? 'none' : 'flex')
            this.publicAddressesElements.forEach(Element => Element.style.display = isPublic ? 'flex' : 'none')
    ...
    selectAdressIndex(index) {
        return () => {
            this.publicPointsCheckBoxesElements.forEach((Element, ind) => Element.innerHTML = this.isSelectedAdressPublic && (ind === index) ? activeRatio : inactiveRatio)
            this.privatePointsCheckBoxesElements.forEach((Element, ind) => Element.innerHTML = !this.isSelectedAdressPublic && (ind === index) ? activeRatio : inactiveRatio)
        }
    }

    confirmSelect() {
        this.isCoisedAdressPublic = this.isSelectedAdressPublic
    ...
    closeModal() {
        this.swichAddressType(isCoisedAdressPublic)()
...
const paymentsControl = new paymentsSelector({
    ...
    cardsRatioButtons: Array(4).fill(0).map((_, ind) => `modal-payment-${ind}-ratio`),
    cardsContainers: ['card-container-right', 'card-container-left'],
    cardsParams: [
        { cardIconSVG: mirCardSVG },
        { cardIconSVG: visaCardSVG },
        { cardIconSVG: masterCardSVG },
        { cardIconSVG: maestroSVG },
    ]
